...
export default function UsersList() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const { data } = await api.get('/users', { params: { q, page, pageSize } });
        setRows(data.data || []);
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-slate-900 text-2xl font-bold">Users</h1>
        <p className="text-slate-500 text-sm">Manage asset users.</p>
      </div>
      <div className="mb-4 flex items-center gap-3">
        <input value={q} onChange={(e)=>{ setQ(e.target.value); setPage(1); }} placeholder="Search name, email, department, job" className="rounded-md border-slate-300 focus:border-primary focus:ring-primary" />
      </div>
            ...
            {rows.map(u => (
              <tr key={u.id} className="border-t">
                ...
                <td className="px-4 py-2 text-right">
                  <Link to={`/users/${u.employee_id}`} className="text-primary text-sm">View</Link>
                </td>
              </tr>
            ))}
            {rows.length === 0 && <tr><td className="px-4 py-6 text-slate-500" colSpan={5}>No users</td></tr>}
          </tbody>
        </table>
      </div>
